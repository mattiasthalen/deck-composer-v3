...
| A5 | ManaBox has two deck-text shapes and this project writes one of them. **Import grammar, what we write, closed:** `// Commander`, `// Mainboard` and `// Maybeboard` sections; a pinned printing is `N Name (SET) CollectorNumber`; a basic or an unowned maybeboard entry may be the bare `N Name`. ManaBox accepts this. **Export shape, what ManaBox emits:** no section headers, no comments, every line pinned, and basics pinned and split across printings — `4 Plains (SOS) 273` / `5 Plains (SOS) 272`. No suffix appears in either shape. | Artifacts | Import grammar verified 2026-09-20 on gen 1's four decks, which ManaBox imported. Export shape verified 2026-09-23 on a ManaBox 4.1.12 deck export of 2026-08-18 held in manabase's eval fixtures. The two were conflated in this row until then. `check` reads the import grammar only and rejects any line outside it at its line number; a raw export fails at line 1, loudly. Round-tripping an export is not intended — what is sleeved reaches the project through the collection export's deck-typed binder rows, not through deck exports | A failed import; a land count that omits basics; a real line in either shape carrying a suffix — verify by exporting one deck containing a foil card, then amend this row first and the parser second; or a need to read a raw export, which reopens the grammar. |
...
| A7 | At most two black decks can sit at one table, and two is comfortable only if neither is black-primary. | Composer, analyzer | Holds across every basic-land estimate the project can construct; the sensitivity table under Measured facts carries the figures. Three black decks is over on every reading. Two leaves 8.8 Swamps spare on an even split, 4.5 if the three-colour seat skews black, and 2.0 if both black seats do — so the spare is a ceiling, not a floor, because an even split is itself the optimistic assumption for a deck with a primary colour **First three-colour measurement, 2026-09-24:** Wick built at 11 Island / 11 Swamp / 7 Mountain against an even-split estimate of 12 each — divergence conservative on Swamp, generous by nothing. The owned Rat core is blue-heavy, so the primary-colour skew A7 feared ran toward the basic with the most headroom. One deck; the risk stands for a black-primary three-colour seat, but the one built did not realise it | Acquiring Swamps; or two black decks both building black-primary, which the checkpoint cannot see and the first built seat will. |
...
| ManaBox 4.1.12 deck export, 2026-08-18 (`manabase/.claude/evals/fixtures/collections/real-deck.txt`) | 20 lines, every one `N Name (SET) CollectorNumber`; no headers, comments or suffixes; basics pinned and split across printings, 4+5 Plains and 4+4 Swamp. Split pinned basics are therefore a real input, and summing them by name is required, not defensive |
...
Source: the first table composed by this project, `decks/bloomburrow-2026-09.table.json`,
2026-09-24, against the pinned export. Wick, the Whorled Mind (UBR), Camellia, the
Seedmiser (BG), Alania, Divergent Storm (UR), Finneas, Ace Archer (GW); built in
`check`'s named order Wick → Camellia → Alania → Finneas; passed with no violations.

| Fact | Value |
|---|---|
| Spreads: lands / average MV / creatures / targeted interaction | 0 / 0.37 / 7 / 6 (36 lands every deck; MV 2.77–3.14; creatures 26–33; interaction 10–16) |
| Swamps used vs estimated at the checkpoint | 26 of 38 used, 29 estimated: divergence −3, and every basic diverged conservative |
| Tribal cores achieved of ceiling | Rat 17/17, Squirrel 15/15, Otter 17/18, Rabbit 17/17 |
| Which seat `check` names first | Wick only where the scarcest basic is one he claims. Where Plains or Forest is tightest, a two-colour seat wins on the name tie-break. Correct per ADR-0005; the composer brief's "Wick first at every viable table" was an over-claim from three tables |
| Red binds as hard as black | Once Wick charges Mountains, no all-Bloomburrow table with three strong, distinct tribal cores fits with fewer than two black decks. Every strong table is exactly two black decks — the ceiling A7 sets is also the floor |
