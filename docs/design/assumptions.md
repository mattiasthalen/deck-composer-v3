...
| A7 | At most two black decks can sit at one table. | Composer, analyzer | **Robust across the modelling range, which is stronger than either model alone.** The two estimates differ only in how many of a deck's 35 land slots go to nonbasics: the design analysis assumed 6.5 per deck, the tool assumes 0, and gen 1's four real decks absorbed 2, 2, 6, 2. Swamp demand against 38 owned — two black decks 23.8 / 26.7 / 29.2 across that range, three black decks 38.0 / 42.7 / 46.7. Three is over on every reading but the design analysis's own, where it lands exactly on 38 of 38; two has at least 9 spare under the most pessimistic. Both models share the even-split prior, so agreement between them is not independent confirmation — the design analysis was simply the more optimistic about absorption, by roughly a factor of two against the only real evidence, which is the dangerous direction. Separately measured once on gen 1's four two-colour decks: conservative on four basics, exact on one, generous on none, with nonbasic absorption accounting for most of that margin. No claim about the model's direction generally follows. Three-colour is unmeasured and the arithmetic runs the other way: an even split predicts 11.7 per basic for Wick at UBR where a black-primary base wants 15 to 18. Gen 1's builder had no even-split rule at all, only a colour-coverage test, yet chose exactly even in two decks of four — it shares the prior without sharing the mechanism | Acquiring Swamps; or a three-colour deck's measured basics diverging generous enough to put a two-black table over. |
