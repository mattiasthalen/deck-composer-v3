...
| A7 | At most two black decks can sit at one table. | Composer, analyzer | Derived from 38 Swamps against a 35-land floor with Wick mandatory, assuming an even split of basics across each deck's colours. **Measured once, on gen 1's four two-colour decks: conservative on four basics, exact on one, generous on none — but nonbasic lands absorbing 2 to 6 slots per deck account for most of that margin, and stripping it leaves a split error of roughly ±2.** No claim about the model's direction generally follows. Three-colour is unmeasured, and arithmetic says it errs the other way: for Wick at UBR an even split predicts 11.7 per basic, where a black-primary base wants 15 to 18, generous by 3 to 6 on the primary colour. Gen 1's builder had no even-split rule — only a colour-coverage test — yet chose exactly even in two decks of four, so it shares the estimate's prior without sharing its mechanism | Acquiring Swamps; or the first three-colour deck's measured basics diverging generous. |
